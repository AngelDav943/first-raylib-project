...
void Draw3DAxis(Vector3 position, Color targetColor, float size = 0.3f)
{
	DrawLine3D(
...
private:
	...
	void getOBBVertices(Vector3 *vertices)
	{
		...
		for (int i = 0; i < 8; i++)
		{
			...
			vertices[i] = Vector3Add(rotatedVertex, currentPosition);
		}
	}
...
public:
	...
	Vector3 getClosestCorner(const Vector3 point)
	{
		Vector3 vertices[8];
		getOBBVertices(vertices);
	...
	void DrawOBBVerts()
	{
		...
		Draw3DAxis(verticesA[0], BLACK);
		Draw3DAxis(verticesA[1], BLUE);
		Draw3DAxis(verticesA[2], ORANGE);
		Draw3DAxis(verticesA[3], GREEN);
		Draw3DAxis(verticesA[4], PURPLE);
		Draw3DAxis(verticesA[5], YELLOW);
		Draw3DAxis(verticesA[6], BROWN);
		Draw3DAxis(verticesA[7], RED);

		for (int i = 0; i < 8; i++)
		{
			DrawLine3D(verticesA[i], verticesA[(i + 1) % 8], BLACK);
		}
	}
